...
namespace _3
{
    class Point
    {
        ...
        public static double SquareDist(Point a, Point b)
        {
            var ans = a.coordinates
                .Zip(b.coordinates, (x, y) => (x - y) * (x - y))
                .Sum();
            return ans;
        }

        public static double SquareDist(Point a, List<Point> b)
        {
            var ans = b.Select(x => SquareDist(a, x)).Min();
            return ans;
        }
